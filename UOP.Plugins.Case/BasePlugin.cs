...
namespace UOP.Plugins.Case
{
    ...
    public interface ILocalPluginContext
    {
        ...
        void ClearPluginTraceLog(ILocalPluginContext localContext, string className);
    }
    ...
    public class LocalPluginContext : ILocalPluginContext
    {
        ...
        public void ClearPluginTraceLog(ILocalPluginContext localContext, string className)
        {
            QueryExpression pluginTraceLogsQuery = new QueryExpression("plugintracelog")
            {
                ColumnSet = new ColumnSet(true)
            };

            localContext.OrganizationService.RetrieveMultiple(pluginTraceLogsQuery)
                .Entities
                .Select(e => e.ToEntity<PluginTraceLog>())
                .Where(t => t.TypeName == className)
                .ToList()
                .ForEach(x => localContext.OrganizationService.Delete("plugintracelog", x.Id));
        }
    }
}
